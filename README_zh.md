...
Akiba 是一个基于 Ghidra 的支持批量流水线任务的二进制分析框架，可为二进制文件分析提供高度灵活性。它基于 Ghidra 和 Kotlin 语言开发，非常适合需要进行大规模批量化分析的场景。目前 Akiba 已实现 Agent 基座，在单文件处理能力上也有了较大提升。
...
- 已完成 LLM Agent 基础基座搭建，可借助大模型完成复杂分析任务
